...
%\usepackage{lineno}
% \usepackage{cancel}
% \newcommand\removed[1]{\renewcommand\CancelColor{\color{red}}\colorbox{yellow}{\cancel{#1}}}
\usepackage{soul}
\setstcolor{red}
\newcommand\edited[1]{\colorbox{yellow}{#1}}
\newcommand\comment[1]{\colorbox{blue!30}{[{\tiny #1}]}}
\newcommand\removed[1]{\colorbox{red!30}{\st{#1}}}
%\linenumbers
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
We present a dispersive analysis with the aim to extract the $\Upsilon$-p scattering length from $\gamma p \to \Upsilon p$ experiments. In this framework, the imaginary part of the $\Upsilon$-p forward scattering amplitude is obtained from $\gamma p \to \Upsilon p$ cross section measurements, and is constrained at high energies from existing HERA \edited{and LHC} data. Its real part is calculated through a once-subtracted dispersion relation, and    
...
In Section 3, we will constrain the high-energy region from existing HERA \edited{and LHC} data. As no data are available so far in the threshold region, we will consider several scenarios for the scattering length providing a range of estimates for the subtraction constant. 
...
\label{eq:disp}
\eeq
with $T_{\Upsilon p}(0) $ the subtraction constant at $\nu = 0$. In this work, the subtraction constant \removed{will}\edited{is suggested to}\comment{somehow not to confuse with fitting the actual data} be obtained by performing a fit of the 
...
In this section, we discuss the fit of the elastic and inelastic discontinuities, which are parameterized according to the three-parameter forms of Eq.~(\ref{eq:discx}), to available  $\Upsilon$-p photo-production data, following \removed{the same}\edited{similar} strategy as our previous analysis for 
...
%We obtain for the elastic photo-production total cross section data \input{chisqr.tex}
...
\edited{The red solid curve, corresponding to $T_{\Upsilon p}(0)=87$,}
\edited{passes through the central value of the data point exactly,}
\edited{according to our constraint on the $C_\mathrm{inel}$.}
}
\label{fig:bslope}
\end{figure}


We show this \removed{fitting procedure to} \edited{constraint based} \edited{on} the $t$-slope parameter $B$ in Fig.~\ref{fig:bslope}, and illustrate \removed{it} \edited{the calculation procedure} first for the $J/\psi$ case (upper panel).
\comment{Perhaps we should somehow say the equations mentioned further,}
\comment{are to be read by replacing ``$\Upsilon$'' to ``$J/\psi$'' therein.}
...
For the $\gamma p \to \Upsilon p$ cross section, we use the dispersive model discussed above with $t$-dependence as in Eq.~(\ref{eq:bdef}). We show results for the three scenarios \removed{discussed above} for the subtraction constant $T_{\Upsilon p}(0)$, and corresponding s-wave scattering length $a_{\Upsilon p}$ \removed{as shown} in Table~\ref{tab:scattlength}. 
...
\hline
1 \input{table2_eic1.tex}
\hline
2 \input{table2_eic2.tex}
\hline
\hline
\end{tabular*}
\caption{Values of 
...
\includegraphics[width=0.49\textwidth]{{dsdt_y_eic1_iW_26}.pdf}
