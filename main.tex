...
are then fitted to the available data points for the elastic $\Upsilon$ photo-production total cross section, as shown in Fig.~\ref{fig:sigmatot} (upper panel), yielding the parameter values shown in Table~\ref{tab:fits} (first column).  
...
We show this \removed{fitting procedure to} \edited{constraint based} \edited{on} the $t$-slope parameter $B$ in Fig.~\ref{fig:bslope}, and illustrate it first for the $J/\psi$ case (upper panel). In solving Eq.~(\ref{eq:brel}) for $B$, 
